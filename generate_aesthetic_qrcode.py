...
def parse_arguments() -> Namespace:
    ...
    parser.add_argument(
        "--qrcode_image_path",
        type=str,
        default="images/QR_frame_01.jpg",
    )
    parser.add_argument(
        "--content_image_path",
        type=str,
        default="images/286_frame_01.jpg",
    )
    parser.add_argument(
        "--style_image_path",
        type=str,
...
def optimize_code(
    ...
    for i in tqdm(range(iterations)):
        ...
        if i %10 == 0:
            asethetic_qrcode = add_position_pattern(
                                    convert_normalized_tensor_to_np_image(x),
                                    convert_normalized_tensor_to_np_image(qrcode_image),
                                    module_size=module_size,
                                    module_num=module_num,
                                )
            asethetic_qrcode = to_pil_image(convert_normalized_tensor_to_np_image(x))
            output_path = f'results/image_{i}.jpg'
            asethetic_qrcode.save(output_path)
...
if __name__ == "__main__":
    ...
    qrcode_image = Image.open(args.qrcode_image_path).resize(qrcode_size, Image.LANCZOS).convert("RGB")
